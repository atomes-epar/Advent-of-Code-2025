...
p2_visits <- sapply(1:nrow(p2_conns), function(x) {
  if(any(p2_conns[x,]=="dac") & any(p2_conns[x,]=="fft")) 1 else 0
})
...
#while(nrow(starts) > 0){
  ...
  starts <- starts |> select(start, ends_with(as.character(n_cons))) |> distinct()
  #circular_cons <- sapply(1:nrow(starts), function(x) is.circular(starts[x,]))
  #if(any(circular_cons)) starts <- starts[-which(circular_cons),]
#}